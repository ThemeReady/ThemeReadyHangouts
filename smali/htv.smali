.class public final Lhtv;
.super Lhty;
.source "SourceFile"


# direct methods
.method public constructor <init>(Landroid/content/Context;Lgzs;)V
    .locals 1

    .prologue
    .line 1
    const/4 v0, 0x0

    invoke-direct {p0, p1, p2, v0}, Lhty;-><init>(Landroid/content/Context;Lgzs;Z)V

    .line 2
    return-void
.end method

.method public static a(Landroid/content/Context;)Landroid/graphics/Bitmap;
    .locals 2

    .prologue
    .line 15
    invoke-virtual {p0}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object v0

    sget v1, Lcom/google/android/apps/hangouts/hangout/StressMode;->AG:I

    invoke-static {v0, v1}, Landroid/graphics/BitmapFactory;->decodeResource(Landroid/content/res/Resources;I)Landroid/graphics/Bitmap;

    move-result-object v0

    return-object v0
.end method


# virtual methods
.method public a(Landroid/widget/ImageView;Lhxe;I)V
    .locals 6

    .prologue
    .line 3
    invoke-static {p2}, Lcom/google/android/apps/hangouts/hangout/StressMode;->a(Lhxe;)Z

    move-result v0

    if-nez v0, :cond_0

    .line 8
    :goto_0
    return-void

    .line 5
    :cond_0
    new-instance v0, Lhtw;

    invoke-interface {p2}, Lhxe;->a()Ljava/lang/String;

    move-result-object v3

    .line 6
    invoke-interface {p2}, Lhxe;->d()Ljava/lang/String;

    move-result-object v4

    move-object v1, p0

    move-object v2, p1

    move v5, p3

    invoke-direct/range {v0 .. v5}, Lhtw;-><init>(Lhtv;Landroid/widget/ImageView;Ljava/lang/String;Ljava/lang/String;I)V

    .line 7
    invoke-virtual {p0, v0}, Lhtv;->a(Lhtz;)V

    goto :goto_0
.end method

.method protected a(Lhtz;Landroid/graphics/Bitmap;)V
    .locals 2

    .prologue
    .line 9
    if-nez p2, :cond_0

    .line 10
    iget-object v0, p1, Lhtz;->f:Landroid/widget/ImageView;

    .line 11
    iget-object v1, p0, Lhtv;->b:Landroid/content/Context;

    invoke-static {v1}, Lhtv;->a(Landroid/content/Context;)Landroid/graphics/Bitmap;

    move-result-object v1

    invoke-virtual {v0, v1}, Landroid/widget/ImageView;->setImageBitmap(Landroid/graphics/Bitmap;)V

    .line 14
    :goto_0
    return-void

    .line 13
    :cond_0
    invoke-super {p0, p1, p2}, Lhty;->a(Lhtz;Landroid/graphics/Bitmap;)V

    goto :goto_0
.end method
